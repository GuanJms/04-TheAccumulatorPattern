...
#                  count = 0
#                  if math.cos(r) > 0:      if math.cos(r+1):
#                      count = 1                count = count + 1
#                  return count             return count
#
#     f. The code in the "graphics accumulation" example below includes:
#            for i_ in range(n):
#        What does the   i_   (underscore) mean?
#           still looping but not using the specific value.
...
#        and one is WRONG.  Which is the WRONG one? The second one. It updates x by diameter AND by multiple of k
